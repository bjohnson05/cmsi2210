...
;  Note that this version uses the "syscall" method, NOT the "C" library "printf()" method
;     so we have to make the program by using the LOADER "ld" instead of "gcc".
;
;  to assemble:   nasm -fmacho64 starsMac.nasm
;  to link:       ld -macosx_version_min 10.6 -no_pie starsMac.o -o starsMac
;  to run:        ./starsMac
; ----------------------------------------------------------------------------------------
...
line:     mov       byte [rdx], '*'    ; load a single star
